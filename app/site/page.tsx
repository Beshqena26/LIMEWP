...
const tabList = [
  ...
  { name: "Activity", icon: "M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z", color: "teal" },
...
function SitePageContent() {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case "activity": return <ActivityTab siteId={siteName} />;
      ...
      <div className={`relative rounded-2xl border mb-6 ${
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <div className="relative" ref={quickActionsRef}>
                <button
                  ...
                  className="inline-flex items-center text-sm font-semibold rounded-xl px-3 h-9 gap-1.5 transition-all text-white bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 shadow-lg shadow-emerald-500/25"
                >
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 13.5l10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z" /></svg>
                  Quick Actions
                  <svg className={`w-3 h-3 transition-transform ${quickActionsOpen ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" /></svg>
                </button>
                {quickActionsOpen && (<>
                  <div className="fixed inset-0 z-10" onClick={() => setQuickActionsOpen(false)} />
                  <div className={`absolute right-0 top-full mt-2 z-20 p-1.5 rounded-xl min-w-[200px] ${isLight ? "bg-white border border-slate-200 shadow-xl" : "bg-[var(--gradient-card-from)] border border-[var(--border-tertiary)] shadow-2xl shadow-black/20"}`}>
                    {[
                      { key: "backup", label: "Backup Now", icon: "M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5", color: "text-violet-500" },
                      { key: "clear-cache", label: "Clear Cache", icon: "M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15", color: "text-emerald-500" },
                      { key: "restart-php", label: "Restart PHP", icon: "M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99", color: "text-orange-500" },
                      { key: "ssl-check", label: "SSL Check", icon: "M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z", color: "text-sky-500" },
                      { key: "security-scan", label: "Security Scan", icon: "M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z", color: "text-purple-500" },
                      { key: "restart-server", label: "Restart Server", icon: "M5.636 5.636a9 9 0 1012.728 0M12 3v9", color: "text-rose-500" },
                    ].map((item) => (
                      <button
                        key={item.key}
                        onClick={() => { setQuickActionsOpen(false); handleQuickAction(item.key); }}
                        className={`w-full flex items-center gap-2.5 rounded-lg px-3 py-2 text-left transition-all ${isLight ? "hover:bg-slate-50" : "hover:bg-white/[0.04]"}`}
                      >
                        <svg className={`w-4 h-4 ${item.color}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d={item.icon} /></svg>
        ...
        <div className={`relative border-t px-4 sm:px-6 py-3 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0 ${
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2 md:gap-6 text-xs">
            ...
            <span className={`hidden md:inline ${isLight ? "text-slate-300" : "text-slate-700"}`}>&bull;</span>
            ...
            <span className={`hidden md:inline ${isLight ? "text-slate-300" : "text-slate-700"}`}>&bull;</span>
            <span className="flex items-center gap-1.5 text-slate-500">
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M17.25 6.75L22.5 12l-5.25 5.25m-10.5 0L1.5 12l5.25-5.25m7.5-3l-4.5 16.5" /></svg>
              PHP: <span className={isLight ? "text-slate-700" : "text-slate-300"}>8.3</span>
            </span>
            <span className={`hidden md:inline ${isLight ? "text-slate-300" : "text-slate-700"}`}>&bull;</span>
            <span className="flex items-center gap-1.5 text-slate-500">
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25" /></svg>
              WP: <span className={isLight ? "text-slate-700" : "text-slate-300"}>6.7.1</span>
            </span>
            <span className={`hidden md:inline ${isLight ? "text-slate-300" : "text-slate-700"}`}>&bull;</span>
            <span className="flex items-center gap-1.5 text-slate-500">
          ...
          <button onClick={() => setActiveTab("analytics")} className="flex items-center gap-1.5 text-xs text-emerald-400 hover:text-emerald-300 font-semibold transition-colors">
            View Site Analytics
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3" /></svg>
          </button>
        </div>
      </div>
