    ...
    >
      ...
      <div className="flex-1 min-w-0 pt-0.5">
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
          <div className={`flex items-center gap-1.5 text-[11px] flex-shrink-0 ${isLight ? "text-zinc-500" : "text-zinc-500"}`}>
            <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            {activity.time}
          </div>
          <span className={`hidden sm:inline ${isLight ? "text-zinc-300" : "text-zinc-600"}`}>•</span>
          <span className={`text-[11px] truncate ${isLight ? "text-zinc-500" : "text-zinc-500"}`}>{activity.details}</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-1.5 pt-1 flex-shrink-0 md:opacity-0 md:group-hover:opacity-100 md:translate-x-2 md:group-hover:translate-x-0 transition-all">
        <button onClick={(e) => { e.stopPropagation(); onViewDetails?.(); }} className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${
          isLight ? "bg-zinc-100 hover:bg-zinc-200 active:bg-zinc-200" : "bg-[#27272A]/70 hover:bg-[#3F3F46] active:bg-[#3F3F46]"
        }`}>
