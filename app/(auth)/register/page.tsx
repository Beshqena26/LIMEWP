...
function getPasswordStrength(password: string): { level: PasswordStrength; score: number } {
  if (!password || password.length < 8) return { level: "weak", score: 1 };
  const hasNumber = /[0-9]/.test(password);
  const hasSpecial = /[^a-zA-Z0-9]/.test(password);
  const hasUpper = /[A-Z]/.test(password);
  if (hasNumber && hasSpecial && hasUpper) return { level: "strong", score: 3 };
  if ((hasNumber && hasUpper) || (hasNumber && hasSpecial) || (hasUpper && hasSpecial))
    ...
    return (
      <div className="text-center">
        ...
        <div className={`w-20 h-20 rounded-2xl mx-auto mb-6 flex items-center justify-center ${
          ...
          <svg className={`w-10 h-10 ${isLight ? "text-slate-400" : "text-slate-500"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
  ...
  return (
    <div className={shaking ? "animate-shake" : ""}>
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-sm text-slate-500 mt-1">Start managing your WordPress sites</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit, onError)} className="space-y-4">
        ...
        <div>
          ...
          {errors.password && <p className="text-red-400 text-[13px] mt-1">{errors.password.message}</p>}
          ...
          {passwordValue.length > 0 && (
            <div className="mt-2">
              ...
              <div className={`h-1.5 rounded-full overflow-hidden ${isLight ? "bg-slate-200" : "bg-[var(--bg-elevated)]"}`}>
                <div
                  className={`h-full rounded-full transition-all duration-300 ${STRENGTH_CONFIG[strength.level].bg} ${STRENGTH_CONFIG[strength.level].width}`}
                />
              </div>
            </div>
          )}
        ...
        <div>
          <label className="flex items-start gap-2.5 cursor-pointer">
            <input
              type="checkbox"
              {...register("terms")}
              className={`w-4 h-4 rounded border mt-0.5 transition-colors flex-shrink-0 ${
                isLight
                  ? "border-slate-300 accent-slate-800"
                  : "border-slate-600 accent-slate-300 bg-[var(--bg-elevated)]"
              }`}
            />
            <span className={`text-sm leading-snug ${isLight ? "text-slate-600" : "text-slate-400"}`}>
              I agree to the{" "}
              <Link href={ROUTES.TERMS} className={`font-medium transition-colors ${isLight ? "text-slate-700 hover:text-slate-900" : "text-slate-300 hover:text-slate-100"}`}>
                Terms of Service
              </Link>{" "}
              and{" "}
              <Link href={ROUTES.PRIVACY} className={`font-medium transition-colors ${isLight ? "text-slate-700 hover:text-slate-900" : "text-slate-300 hover:text-slate-100"}`}>
                Privacy Policy
              </Link>
            </span>
          </label>
          {errors.terms && <p className="text-red-400 text-[13px] mt-1">{errors.terms.message}</p>}
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <button
          type="button"
          onClick={() => showToast.info("GitHub signup coming soon")}
          className={`h-11 rounded-xl border flex items-center justify-center gap-2 text-sm font-medium transition-colors ${
            isLight
              ? "border-slate-200 text-slate-700 hover:bg-slate-50"
              : "border-[var(--border-tertiary)] text-slate-300 hover:bg-[var(--bg-elevated)]"
