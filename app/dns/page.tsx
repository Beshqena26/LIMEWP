    ...
    <AppShell>
      ...
      <div className="flex justify-between items-start mb-8">
        <div>
          ...
          <p className={`text-sm ${isLight ? "text-slate-600" : "text-slate-500"}`}>Configure DNS records for your domains</p>
        </div>
      ...
      <div className="grid grid-cols-12 gap-4 mb-6">
        ...
        <div className={`col-span-8 rounded-2xl p-5 ${cardBase}`}>
          <div className="flex items-center justify-between mb-5">
            <Dropdown>
              <DropdownTrigger>
                <button className={`flex items-center gap-3 rounded-xl px-4 py-2.5 transition-colors ${
                  isLight ? "bg-slate-50 hover:bg-slate-100" : "bg-slate-900/50 hover:bg-slate-800/80"
                }`}>
                  <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${
                    isLight ? "bg-slate-200" : "bg-slate-700"
                  }`}>
                    <svg className={`w-4 h-4 ${isLight ? "text-slate-600" : "text-slate-300"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                  ...
                  <div className="text-left">
                    ...
                    <div className={`font-semibold ${isLight ? "text-slate-800" : "text-slate-100"}`}>{selectedDomain}</div>
                  </div>
                  <svg className={`w-4 h-4 ml-2 ${isLight ? "text-slate-400" : "text-slate-500"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
            ...
            <div className="flex items-center gap-2">
              <span className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg ${
                ...
                <span className={`text-[11px] font-semibold ${isLight ? "text-slate-600" : "text-slate-400"}`}>Propagated</span>
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-4 gap-4">
            {[
              ...
              <div key={stat.label} className={`p-4 rounded-xl ${isLight ? "bg-slate-50" : "bg-slate-900/50"}`}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className={`text-sm font-semibold ${isLight ? "text-slate-800" : "text-slate-100"}`}>{stat.value}</div>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className={`col-span-4 rounded-2xl p-5 ${cardBase}`}>
          <div className="flex items-center justify-between mb-4">
            <h3 className={`text-sm font-semibold ${isLight ? "text-slate-800" : "text-slate-100"}`}>Global Propagation</h3>
            <span className={`text-xs font-bold px-2 py-1 rounded-lg ${
              isLight ? "text-slate-600 bg-slate-100" : "text-slate-300 bg-slate-800"
            }`}>100%</span>
          </div>

          {/* Progress Bar */}
          <div className={`h-2 rounded-full mb-4 overflow-hidden ${isLight ? "bg-slate-200" : "bg-slate-800"}`}>
            <div className={`h-full w-full rounded-full ${isLight ? "bg-slate-500" : "bg-slate-400"}`} />
          </div>
          ...
          <div className="space-y-2.5">
            {propagationServers.map((server, idx) => (
              <div key={idx} className={`flex items-center justify-between p-2.5 rounded-lg ${
                ...
                <div className="flex items-center gap-2">
                  <span className={`text-xs ${isLight ? "text-slate-500" : "text-slate-500"}`}>{server.latency}</span>
          ...
          return (
            ...
            >
              ...
              <span className={`text-xs px-1.5 py-0.5 rounded ${
                isActive
                  ? isLight ? "bg-slate-700 text-slate-200" : "bg-slate-200 text-slate-700"
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 flex-1">
            <div className="relative flex-1 max-w-md">
              <svg className={`absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 ${isLight ? "text-slate-400" : "text-slate-500"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
              ...
              <input
                ...
                className={`w-full pl-10 pr-4 py-2.5 rounded-xl text-sm transition-all outline-none ${
                  isLight
                    ? "bg-slate-100 text-slate-800 placeholder:text-slate-400 focus:bg-slate-50 focus:ring-2 focus:ring-slate-300"
                    : "bg-slate-900/50 text-slate-200 placeholder:text-slate-500 focus:bg-slate-900 focus:ring-2 focus:ring-slate-600"
                }`}
              />
            </div>
            {typeFilter !== "All" && (
      ...
      <div className={`rounded-2xl overflow-hidden ${cardBase}`}>
        ...
        <div className={`grid grid-cols-12 gap-4 px-5 py-3 text-xs font-semibold uppercase tracking-wider ${
          isLight ? "bg-slate-50 text-slate-500 border-b border-slate-200" : "bg-slate-900/80 text-slate-500 border-b border-slate-800"
        }`}>
        ...
        <div className="divide-y divide-slate-200 dark:divide-slate-800">
          {filteredRecords.length === 0 ? (
            <div className={`px-5 py-12 text-center ${isLight ? "text-slate-500" : "text-slate-500"}`}>
              ...
              return (
                ...
                >
                  ...
                  <div className="col-span-2">
                    <span className={`font-medium ${isLight ? "text-slate-800" : "text-slate-200"}`}>{record.name}</span>
                  </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 ${
              ...
              <svg className={`w-5 h-5 ${isLight ? "text-slate-500" : "text-slate-400"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9.879 7.519c1.171-1.025 3.071-1.025 4.242 0 1.172 1.025 1.172 2.687 0 3.712-.203.179-.43.326-.67.442-.745.361-1.45.999-1.45 1.827v.75M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-9 5.25h.008v.008H12v-.008z" />
              </svg>
            </div>
            <div>
              <h3 className={`text-sm font-semibold ${isLight ? "text-slate-800" : "text-slate-100"}`}>Need help with DNS configuration?</h3>
              <p className={`text-sm ${isLight ? "text-slate-500" : "text-slate-500"}`}>Check our documentation for common setups and troubleshooting guides.</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
